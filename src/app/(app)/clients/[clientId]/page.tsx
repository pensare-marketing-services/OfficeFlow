...
export default function ClientIdPage() {
    ...
    const handleMonthNameChange = async (oldName: string, newName: string) => {
        ...
        try {
          ...
          // 6. Update associated GMB METRICS
          const gmbQuery = query(
            collection(db, `clients/${clientId}/gmbMetrics`),
            where('month', '==', oldName)
          );
          const gmbSnap = await getDocs(gmbQuery);
          gmbSnap.forEach(docSnap => {
            batch.update(docSnap.ref, { month: newName });
          });
    ...
    const handleDeleteMonth = async (monthName: string) => {
        try {
            ...
            const gmbQuery = query(collection(db, `clients/${clientId}/gmbMetrics`), where('month', '==', monthName));
            const gmbSnapshot = await getDocs(gmbQuery);
            gmbSnapshot.forEach(doc => batch.delete(doc.ref));
